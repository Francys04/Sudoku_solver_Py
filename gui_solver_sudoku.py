"""generate a example of sudoku with list of lists,import numpy as nup"""
...
def Print(matrix):
    ...
    # use hotkey that press every num
    # click right spot to start the read program
    for num in str_fin:
        pag.press(num)
        pag.hotkey('right')
        counter.append(num)
        # read every rows and column use pyautogui with hotkeys
        # move to down and left of 8 times for every rows
        if len(counter) % 9 == 0:
            pag.hotkey('down')
            pag.hotkey('left')
            pag.hotkey('left')
...
print("The program finish to solve")
